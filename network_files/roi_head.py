...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    labels = torch.cat(labels, dim=0)#[BA]
    regression_targets = torch.cat(regression_targets, dim=0)#[BA,4]
    ...
    sampled_pos_inds_subset = torch.where(torch.gt(labels, 0))[0]#获取非背景的proposal索引

    # 返回标签类别大于0位置的类别信息
    labels_pos = labels[sampled_pos_inds_subset]#类别的索引

    # shape=[num_proposal, num_classes]
    N, num_classes = class_logits.shape
    box_regression = box_regression.reshape(N, -1, 4)#[num_proposal,num_classes,4]
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            if gt_boxes_in_image.numel() == 0:  # 该张图像中没有gt框，为背景
                ...
                matched_idxs_in_image = self.proposal_matcher(match_quality_matrix)#num_proposal个与gt之间IOU最大的的proposal对应gt的索引
                #因为将gt拼接到了proposals中所以肯定能保证所有的gt都被proposal被匹配
    ...
    def subsample(self, labels):
        ...
        return sampled_inds#所有采样的样本的，包括正负样本的索引
    ...
    def select_training_samples(self,
        ...
        # 将gt_boxes拼接到proposal后面
        proposals = self.add_gt_proposals(proposals, gt_boxes)#避免初始化rpn产生的proposal匹配不到任何一个gt根据iou

        # get matching gt indices for each proposal
        # 为每个proposal匹配对应的gt_box，并划分到正负样本中
        #matched_idxs为gtbox的索引，labels为对应gt的类别
        matched_idxs, labels = self.assign_targets_to_proposals(proposals, gt_boxes, gt_labels)
        # sample a fixed proportion of positive-negative proposals
        # 按给定数量和比例采样正负样本
        sampled_inds = self.subsample(labels)#每一张图片的正负样本proposal索引
        matched_gt_boxes = []
        ...
        regression_targets = self.box_coder.encode(matched_gt_boxes, proposals)#根据匹配的正样本gt和proposal计算相对坐标值
    ...
    def postprocess_detections(self,
        ...
        pred_boxes = self.box_coder.decode(box_regression, proposals)#绝对坐标[NA,C,4]

        # 对预测类别结果进行softmax处理
        pred_scores = F.softmax(class_logits, -1)#[NA,C]
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            labels = labels.view(1, -1).expand_as(scores)#[A,C]

            # remove prediction with the background label
            # 移除索引为0的所有信息（0代表背景）
            boxes = boxes[:, 1:]#[A,C-1,4]
            scores = scores[:, 1:]#[A,C-1]
            labels = labels[:, 1:]#[A,C-1]

            # batch everything, by making every class prediction be a separate instance
            boxes = boxes.reshape(-1, 4)#[A*C-1,4]
            scores = scores.reshape(-1)#[AC]
            labels = labels.reshape(-1)#[AC]
    ...
    def forward(self,
        ...
        result = torch.jit.annotate(List[Dict[str, torch.Tensor]], [])#后处理用于推理部分
        losses = {}
        if self.training:
            ...
            #=====================================
            #边界框损失出现nan
            # if torch.any(torch.isnan(loss_box_reg)):
            #     print(targets)
            #=====================================
            losses = {
