...
class BoxCoder(object):
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        dw = torch.clamp(dw, max=self.bbox_xform_clip)#解码防止exp太大
...
class Matcher(object):
    ...
    def __call__(self, match_quality_matrix):
        ...
        # matches对应最大值所在的行索引
        matched_vals, matches = match_quality_matrix.max(dim=0)  # the dimension to reduce.对每一列求最大值->可能存在没有任何一个gt被以及多个anchor匹配一个gt的情况
        ...
        between_thresholds = (matched_vals >= self.low_threshold) & (
            matched_vals < self.high_threshold#获取蒙版
        )
