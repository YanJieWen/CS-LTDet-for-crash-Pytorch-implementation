...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        #step1:归一化以及等比例缩放
        images, targets,_,_ = self.transform(images, targets)  # 对图像进行预处理
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,#需要类别+1
                 # transform parameter
                 ...
                 rpn_batch_size_per_image=256, rpn_positive_fraction=0.5,  # rpn计算损失时采样的样本数，以及正样本占总样本的比例，与2000个proposals没有交叉
        ...
        if rpn_anchor_generator is None:
            anchor_sizes = ((32,), (64,), (128,), (256,), (512,))#低到高的特征图尺寸设置高到低的anchor用于小到大的目标检测，如果只有
            #一张特征图上设置15个anchors不利于不同尺寸的目标检测
        ...
        if rpn_head is None:
            rpn_head = RPNHead(
                out_channels, rpn_anchor_generator.num_anchors_per_location()[0]#每个预测特征图每个窗口被分配的anchors数目
            )
        ...
        super(FasterRCNN, self).__init__(backbone, rpn, roi_heads, transform)#父类继承fastercnn
